"""
DeepSeek reasoning model benchmark
Uses deepseek-reasoner with reasoning_effort=medium, max_tokens=32000, non-streaming OpenAI SDK.
...
# Real cloudcast request extracted from adaevolve_20260529_112205.log
MESSAGES = [
    {
        "role": "system",
        "content": (
...
def run() -> dict:
    ...
    if not api_key:
        raise EnvironmentError("Set DEEPSEEK_API_KEY env var")

    client = OpenAI(api_key=api_key, base_url=API_BASE)

    input_chars = sum(len(m["content"]) for m in MESSAGES)
    print(f"Model            : {MODEL}")
    print(f"Reasoning effort : {REASONING_EFFORT}  max_tokens={MAX_TOKENS}")
    ...
    # Token breakdown from API (exact, not estimated)
    ...
    # Count SEARCH/REPLACE blocks and code vs explanation
