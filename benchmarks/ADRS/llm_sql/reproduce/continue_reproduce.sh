#!/usr/bin/env bash
# LLM SQL — resume all 4 KE ablation cases from their latest checkpoint.
set -euo pipefail
...
continue_case() {
  ...
  if [[ -z "$ckpt" ]]; then
    echo "== llm_sql / $case: no checkpoint found, skipping =="
    return
  fi
  run_dir=$(dirname "$(dirname "$ckpt")")
  echo "== llm_sql / $case: resuming from ${ckpt##*/} → output: $run_dir =="
...
wait
echo "llm_sql continue_reproduce: all 4 cases finished."
