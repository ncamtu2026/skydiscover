"""Skydiscover-compatible evaluator for cant_be_late / mixed_availability_loose_deadline_small_overhead.
...
_PROBLEM_ID    = "cant_be_late/mixed_availability_loose_deadline_small_overhead"

# Reuse a single runner across evaluate() calls (lazy Docker init, no state).
_runner = ResearchDockerRunner(
    base_dir=_FRONTIER_BASE,
    datasets_dir=_DATASETS_DIR,
    timeout=900,  # 300s eval + buffer for Docker startup and uv install
)
