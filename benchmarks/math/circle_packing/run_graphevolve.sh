#!/usr/bin/env bash
# circle_packing — GraphEvolve: policy-driven search over the ExperienceGraph (no islands).
...
run() {
  local ts
  ts=$(date +%m%d_%H%M%S)
  echo "== circle_packing / graphevolve / $ts =="
...
wait
echo "circle_packing graphevolve: finished."
