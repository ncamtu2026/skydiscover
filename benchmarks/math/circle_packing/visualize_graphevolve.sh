#!/usr/bin/env bash
# Visualize circle_packing GraphEvolve runs.
#
# Env vars:
...
while IFS= read -r d; do
  dirs+=("$d")
done < <(ls -d outputs/graphevolve/graphevolve_* 2>/dev/null | sort -r | head -"$LATEST")

if [[ ${#dirs[@]} -eq 0 ]]; then
  echo "No graphevolve runs found under outputs/graphevolve/"
  echo "Run 'bash run_graphevolve.sh' first."
  exit 1
fi
